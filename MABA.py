...
class MABImageAttacker():
    ...
    def variance_based_weights(self, HC_Matrix, temp=0.2, eps=1e-6):
        """
        平滑版方差权重生成器：通过温度系数控制权重分布尖锐度
        Args:
            HC_Matrix: 形状 [Layer, Batch, Seq, Embed]
        ...
        """
        # 展平计算方差
        reshaped = HC_Matrix.view(HC_Matrix.size(0), -1)  # [Layer, ...]
    ...
    def svd_enhanced_rank_constraint(self, matrix):
        """ 对每个layer-batch组合的[Seq, Embed]矩阵进行SVD约束，保留前k个奇异值，压缩剩余部分,DFS module """
        # 合并layer和batch维度
        L, B, S, E = matrix.shape
        reshaped = matrix.view(L*B, S, E)     
        # 批处理SVD
        U, S_vec, Vh = torch.linalg.svd(reshaped, full_matrices=False)
        k = int(S_vec.shape[-1] * 0.05)
        # 核范数计算（仅压缩top-k个奇异值）
        nuclear_loss = S_vec[:, k:].sum(dim=-1)
    ...
    def txt_guided_attack(self, model, imgs, txt2img, all_txt_supervisions,device, scales=None, txt_embeds=None,inter_text_embeddings = None):
        ...
        # Register hook for intermediate representation (DFS module) 
        ...
        for step in range(self.steps):  # self.steps=10
            ...
            # --- AE Generation ---
                ...
                with torch.enable_grad():
                    ...
                    # 计算MIA_Loss
                    MIA_Loss = torch.tensor(0.0, device=device)
                    for i in range(5):
                        MIA_Loss += self.loss_func(adv_embeds[i*b:(i+1)*b], txt_embeds, txt2img, all_txt_supervisions)
                    # 计算MIA_Loss梯度
                    MIA_Loss.backward(retain_graph=True)
                    adv_imgs.retain_grad()
                    mia_grad = adv_imgs.grad.detach().clone()
                    adv_imgs.grad.zero_()
                    # 清除之前的梯度
                    model.zero_grad()
                   # 计算DFS_loss
                    ...
                    HC_Matrix = inter_visual_embeddings[:-1,:,:,:] - one_matrix @ rank1_approx# 
                    nuclear_norm = self.svd_enhanced_rank_constraint(HC_Matrix)
                    ...
                    DFS_loss = -(beta_expo * layer_weights.view(-1, 1)).mean(dim=1).sum()
                    # 计算DFS_loss梯度
                    DFS_loss.backward()
                    adv_imgs.retain_grad()
                    ...
                    adv_imgs.grad.zero_()
                    # 梯度投影
                    sa_flat = mia_grad.flatten()
                    fc_flat = fc_grad.flatten()
                    dot = torch.dot(fc_flat, sa_flat)
                    sa_norm_sq = torch.norm(sa_flat) ** 2 + 1e-6
                    projected_dfs = fc_grad - (dot / sa_norm_sq) * mia_grad

                    # 合并梯度（SA梯度主导 + 投影后的dfs梯度）
                    final_grad = mia_grad + 0.36 * projected_dfs
                ...
                # 空间管理
                del final_grad, mia_grad, fc_grad, DFS_loss, MIA_Loss, perturbation, sa_flat, fc_flat, dot, sa_norm_sq, projected_dfs
                ...
                with torch.enable_grad():
                    ...
                    beta = 2.5
                    beta_expo = torch.pow(torch.abs(log_norm), beta)
                    DFS_loss = -(beta_expo * layer_weights.view(-1, 1)).mean(dim=1).sum(dim=0)
                    DFS_loss.backward()
                    fc_grad = adv_imgs.grad.detach().clone()
                    adv_imgs.grad.zero_()
                    # 步骤4：梯度投影（直接操作对抗图像梯度）
                    mia_grad_flat = mia_grad.flatten()
                    fc_grad_flat = fc_grad.flatten()
                    dot_product = torch.dot(fc_grad_flat, mia_grad_flat)
                    sa_norm_sq = torch.norm(mia_grad_flat)**2 + 1e-6
                    projected_dfs_grad = fc_grad - (dot_product / sa_norm_sq) * mia_grad
                    # 步骤5：合并梯度（对抗梯度主导 + 投影后的正则梯度）
                    final_grad = mia_grad + 0.36 * projected_dfs_grad  # 可调节0.217为beta参数
                    # --- 更新对抗图像（快速无优化器版本）---
                    # 梯度归一化
                    final_grad = final_grad / torch.mean(torch.abs(final_grad), dim=(1,2,3), keepdim=True)
                    # 生成扰动（Sign攻击）
                    ...
                    adv_imgs = adv_imgs.detach()
                    # 空间管理
                    del final_grad, mia_grad, fc_grad, DFS_loss, MIA_Loss, perturbation, mia_grad_flat, fc_grad_flat, dot_product, sa_norm_sq, projected_dfs_grad
                    torch.cuda.empty_cache()
        # 注销 hook，释放内存
